...
# This allows declaration	use Solaris::NDD ':all';
...
sub AUTOLOAD {
	...
	if ($! =~ /Invalid/ || $!{EINVAL}) {
	    ...
	    croak "Your vendor has not defined Solaris::NDD macro $constname";
	}
    }
    {
	no strict 'refs';
...
Solaris::NDD - Perl extension for interacting with Solaris Network Device Driver
interface (NDD). This module allows for getting and setting tunable parameters
...
=head1 SYNOPSIS
  ...
  $ref = new Solaris::NDD( <pseudo-device> );
...
The Solaris::NDD module is a programming interface for viewing and setting the values
of various tunable parameters associated with network drivers. A Solaris::NDD object can 
...
network driver:
   ...
   $ref = new Solaris::NDD('/dev/ip');
   print $ref->{ip_forwarding};     # prints the value of ip_forwarding tunable 
   $ref->{ip_forwarding} = 0;	    # turn off ip forwarding

Once instantiated, a Solaris::NDD object will return a reference to a tied hash, where
